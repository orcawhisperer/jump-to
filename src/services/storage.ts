import { ShortcutMap, Settings, UsageStats, RecentItem, Analytics, HotKey } from '@/types';
...
export class StorageService {
  ...
  static async getSettings(): Promise<Settings> {
    const { settings = this.getDefaultSettings() } = await chrome.storage.local.get('settings');
    return settings;
  }
  ...
  static async getStats(): Promise<UsageStats> {
    const { stats = this.getDefaultStats() } = await chrome.storage.local.get('stats');
    return stats;
  }
  ...
  static async getAnalytics(): Promise<Analytics> {
    const { analytics = { events: [] } } = await chrome.storage.local.get('analytics');
    return analytics;
  }
  ...
  static async setHotKeys(hotkeys: HotKey[]): Promise<void> {
    await chrome.storage.local.set({ hotkeys });
  }

  static async getHotKeys(): Promise<HotKey[]> {
